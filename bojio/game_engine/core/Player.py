...
class Player:
    def __init__(
        self, name: str, print_board_state_func: Callable[[dict, dict, str, str], None]
    ) -> None:
    ...
    def tick(
        ...
        if len(moves) == 0:  # handle checkmate/stalemate
            ...
            if strategy == "naive":  # pick a random move
                ...
                move = moves[idx]
            if (
                strategy == "capturebot5000"
            ):  # prioritises captures over everything else
        ...
        opp_king_sq = [
            (key, opp_pieces[key])
            for key in opp_pieces
            if opp_pieces[key].name == PieceNames.KING
        ]
        assert len(opp_king_sq) == 1, str(
            "Error: Number of opponent kings unexpected: %d" % (len(opp_king_sq))
        )
    ...
    def check_if_under_check(self, my_pieces: dict, opp_pieces: dict) -> bool:
        ...
        squares_attacked_by_opp = self.get_squares_attacked_by_opp(
            my_pieces, opp_pieces
        )
        my_king_sq = [
            key for key in my_pieces if my_pieces[key].name == PieceNames.KING
        ]
        assert len(my_king_sq) == 1, str(
            "Error: Unexpected number of my kings: %d" % (len(my_king_sq))
        )
    ...
    def filter_moves(self, moves: list, my_pieces: dict, opp_pieces: dict) -> list:
        legal_moves = []
        for move in moves:
            my_pieces_copy, opp_pieces_copy = self.soft_execute_move(
                move, my_pieces, opp_pieces
            )
    ...
    def soft_execute_move(
        self, move: str, my_pieces: dict, opp_pieces: dict
    ) -> tuple[dict, dict]:
        ...
        if move[0] in ["R", "B", "N", "Q", "K"]:  # non-pawn
            ...
            king_sq = [
                x for x in my_pieces_copy if my_pieces_copy[x].name == PieceNames.KING
            ]
            assert len(king_sq) == 1, str(
                "Error: Unexpected number of kings = %d" % (len(king_sq))
            )
            assert king_sq[0][0] == "e", str(
                "Error: Trying O-O, king not on e[x] square! = %s" % (king_sq)
            )
            king_row = int(king_sq[0][1])
            rook_sq = str("h%d" % (king_row))
            assert (
                rook_sq in my_pieces_copy
                and my_pieces_copy[rook_sq].name == PieceNames.ROOK
            ...
            assert king_sq[0][0] == "e", str(
                "Error: Trying O-O-O, king not on e[x] square! = %s" % (king_sq)
            )
            king_row = int(king_sq[0][1])
            ...
            if move[2] == "x":
                ...
                if not sq_to in opp_pieces_copy:  # must be an en-passant capture
                    ...
                    assert pawn_to_capture in opp_pieces_copy, str(
    ...
    def check_en_passant_offered(self, move: str, opp_pieces: dict) -> tuple[str, bool]:
        ...
        if move[0] in [
            ...
            "K",
            "O",
        ]:  # only pawn moves can offer en-passant
            return ("x", False)
            ...
            if col == "a":  # edge-case
                ...
                for c in [-1, 1]:
                    if str("%s%d" % (COLUMN_ALPHABETS[idx + c], row_to)) in opp_pieces:
                        if (
                            opp_pieces[
                                str("%s%d" % (COLUMN_ALPHABETS[idx + c], row_to))
                            ].name
                            == PieceNames.PAWN
                        ):
                            return (col, True)
    ...
    def get_all_non_king_moves(
        self, states: dict, my_pieces: dict, opp_pieces: dict
    ) -> tuple[list, list]:
        ...
        pawns = [
            (key, my_pieces[key])
            for key in my_pieces
            if my_pieces[key].name == PieceNames.PAWN
        ]
        pawn_moves, sq_attacked = self.evaluate_pawn_moves(
            pawns, opp_pieces, my_pieces, states["en_passant"]
        )
        ...
        knights = [
            (key, my_pieces[key])
            for key in my_pieces
            if my_pieces[key].name == PieceNames.KNIGHT
        ]
        knight_moves, sq_attacked = self.evaluate_knight_moves(
            knights, opp_pieces, my_pieces
        )
        ...
        bishops = [
            (key, my_pieces[key])
            for key in my_pieces
            if my_pieces[key].name == PieceNames.BISHOP
        ]
        bishop_moves, sq_attacked = self.evaluate_bishop_moves(
            bishops, opp_pieces, my_pieces
        )
        ...
        rooks = [
            (key, my_pieces[key])
            for key in my_pieces
            if my_pieces[key].name == PieceNames.ROOK
        ]
        ...
        queens = [
            (key, my_pieces[key])
            for key in my_pieces
            if my_pieces[key].name == PieceNames.QUEEN
        ]
        queen_moves, sq_attacked = self.evaluate_queen_moves(
            queens, opp_pieces, my_pieces
        )
    ...
    def get_squares_attacked_by_opp(self, my_pieces: dict, opp_pieces: dict) -> list:
        ...
        _, squares_attacked_by_opp = self.get_all_non_king_moves(
            opp_states, opp_pieces, my_pieces
        )
        self.opponent_point_of_view = False
        ...
        assert len(opp_king) == 1, str(
            "Error: Unexpected number of opponent kings: %d" % (len(opp_king))
        )
    ...
    def get_all_legal_moves(
        ...
        king = [
            (key, my_pieces[key])
            for key in my_pieces
            if my_pieces[key].name == PieceNames.KING
        ]
        assert len(king) == 1, str(
            "Error: Unexpected number of my kings: %d" % (len(king))
        )
    ...
    def evaluate_king_moves(
        ...
        for move in possible_moves:
            ...
            if -1 < (col_idx + move[0]) < 8 and 0 < (row + move[1]) < 9:
                new_loc = str(
                    "%s%d" % (COLUMN_ALPHABETS[col_idx + move[0]], row + move[1])
                )
    ...
    def evaluate_queen_moves(
        self, queens: list[tuple], opp_pieces: dict, my_pieces: dict
    ) -> tuple[list, list]:
        moves = []
        squares_attacked = []
        ...
        for queen in queens:
            ...
            for p in possible_directions:
                ...
                while -1 < (col_idx + mult * p[0]) < 8 and 0 < (row + mult * p[1]) < 9:
                    new_loc = str(
                        "%s%d"
                        % (COLUMN_ALPHABETS[col_idx + mult * p[0]], row + mult * p[1])
                    )
                    squares_attacked.append(new_loc)
    ...
    def evaluate_rook_moves(
        self, rooks: list[tuple], opp_pieces: dict, my_pieces: dict
    ) -> tuple[list, list]:
        moves = []
        squares_attacked = []
    ...
    def evaluate_bishop_moves(
        self, bishops: list[tuple], opp_pieces: dict, my_pieces: dict
    ) -> tuple[list, list]:
        moves = []
        squares_attacked = []
    ...
    def evaluate_knight_moves(
        self, knights: list[tuple], opp_pieces: dict, my_pieces: dict
    ) -> tuple[list, list]:
        moves = []
        squares_attacked = []
    ...
    def evaluate_pawn_moves(
        self, pawns: list[tuple], opp_pieces: dict, my_pieces: dict, en_passant: tuple
    ) -> tuple[list, list]:
        ...
        for pawn in pawns:
            ...
            if self.name == "white":
                ...
                pprint(
                    'Error: Please set player name to "white" or "black", currently: %s'
                    % (self.name)
                )
                sys.exit()
                    ...
                    if row in [1, 8]:  # pawn promotion
                        for promoted_piece in ["Q", "B", "N", "R"]:
                            moves.append(
                                str(
                                    "a%dxb%d=%s"
                                    % ((row - direction), row, promoted_piece)
                                )
                            )
                    else:
                ...
                if idx_col in [(idx + 1), (idx - 1)]:
                    if self.name == "white" and row == 6:
                        moves.append(
                            str(
                                "%s%dx%s%d"
                                % (col, (row - direction), en_passant[0], row)
                            )
                        )
            ...
            if (not new_pos in opp_pieces) and (
                not new_pos in my_pieces
            ):  # no piece blocking
                ...
                if row in [1, 8]:  # pawn promotion
                    for promoted_piece in ["Q", "B", "N", "R"]:
                        moves.append(
                            str(
                                "%s%d%s%d=%s"
                                % (col, (row - direction), col, row, promoted_piece)
                            )
                        )
                else:
                ...
                if row in [3, 6]:
                    ...
                    if (not new_pos in opp_pieces) and (
                        ...
                        moves.append(
                            str(
                                "%s%d%s%d"
                                % (col, (row - direction), col, row + direction)
                            )
                        )
