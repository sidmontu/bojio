...
class GridWorld(MDP) :
    ...
    def getReward(self, state, action, nextState) :
        if state in self.rules :
            assert nextState == self.rules[state]['moveTo'],\
...
if __name__ == "__main__" :
    ...
    iterations = 1000
    discount = 0.9
    values = dict(zip(gw.getStates(),[0.]*(n*n)))
        ...
        for state in gw.getStates() :
            ...
            for action in gw.getPossibleActions(state) :
                for next_state, prob in gw.getTransitionStatesAndProbs(state, action) :
                    ...
                    action_values[action] += prob * (gw.getReward(state, action, next_state) + discount * v[next_state])
